...
class OfdmSym(object):
    ...
    def __init__(self, params):
        ...
        # OFDM symbol start and stop indexes
    ...
    def rx(self, syms_td, syms_ref):
        ...
        #syms_fd = np.fft.ifftshift(np.fft.fft(syms_cp, axis=1), axes=1)

        #syms_fd_shifted = np.hstack([syms_fd[:,1:1+self.n_subc/2], syms_fd[:,1+self.n_subc/2+self.n_gb:1+self.n_subc/2+self.n_gb+self.n_subc/2]])
        syms_fd_shifted = np.hstack([syms_fd[:,1+self.n_subc/2+self.n_gb:1+self.n_subc/2+self.n_gb+self.n_subc/2], syms_fd[:,1:1+self.n_subc/2]])

        syms_out_shifted = np.fft.ifftshift(syms_fd, axes=0)
